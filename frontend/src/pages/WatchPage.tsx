...
import { ORGINAL_IMG_BASE_URL, SMALL_IMG_BASE_URL } from "../utils/constants";
...
const WatchPage = () => {
  ...
  const [similarContent, setSimilarContent] = useState<SimilarContent>({ content: { results: [] } });
  ...
  useEffect(() => {
    const getTrailers = async () => {
      try {
        ...
        setTrailers([]); // Fallback to an empty array on error
      }
    };
    getTrailers();
  }, [contentType, id]);
  ...
  useEffect(() => {
    const getSimilarContent = async () => {
      try {
        ...
        setSimilarContent({ content: { results: [] } });
      }
    };
    getSimilarContent();
  }, [contentType, id]);
  ...
  return <div className='bg-black min-h-screen text-white'>
    <div className='mx-auto container px-4 py-8 h-full'>
      ...
      {similarContent.content.results && similarContent.content.results.length > 0 && (
        <div className="mt-12 max-w-5xl mx-auto relative">
          <h3 className='text-3xl font-bold mb-4'>Similar Movies/TV Shows</h3>
          <div className='flex overflow-x-scroll scrollbar-hide gap-4 pb-4 group' ref={sliderRef}>
          {similarContent.content.results.map((item) => {
              if (item.poster_path === null) return null;
              return (
                <Link key={item.id} to={`/watch/${item.id}`} className='w-52 flex-none'>
                  <img src={SMALL_IMG_BASE_URL + item.poster_path} alt={`Poster for content ${item.id}`} className="w-full h-auto rounded-md" />
